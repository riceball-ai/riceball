...
const { data: translationData, refresh, pending } = await useAPI<any>(
  `/v1/admin/assistants/${props.assistantId}/translations`,
  { server: false }
)
...
const saveTranslation = async () => {
  ...
  try {
    ...
    await $api(
      `/v1/admin/assistants/${props.assistantId}/translations/${selectedLocale.value}`,
      {
        method: 'PUT',
  ...
  <div class="space-y-6">
    ...
    <template v-else-if="translationData">
      <div class="flex items-center justify-between">
         <div>
             <h3 class="text-lg font-medium">{{ t('admin.pages.assistants.translations.title') }}</h3>
             <p class="text-sm text-muted-foreground">{{ t('admin.pages.assistants.translations.subtitle', { name: assistantData.default_name }) }}</p>
         </div>
      ...
      <div class="grid grid-cols-12 gap-6">
        ...
        <Card class="col-span-3">
          <CardHeader>
            <CardTitle class="text-base">{{ t('admin.pages.assistants.translations.languageList') }}</CardTitle>
          </CardHeader>
