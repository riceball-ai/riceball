  ...
  highlight: function (str: string, lang?: string): string {
    ...
    const buildCodeBlock = (codeHtml: string) => {
      ...
      const svgPanels = showSvgPreview
        ...
        : codeHtml
      return `<div class="code-block-container bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden my-4 max-w-full"><div class="code-block-header flex items-center justify-between px-3 py-1 bg-gray-200 dark:bg-gray-700 border-b border-gray-300 dark:border-gray-600"><span class="text-xs font-medium text-gray-600 dark:text-gray-400">${displayLang}</span>${headerActions}</div><div id="${codeBlockId}" class="code-block-body" data-collapsed="false">${svgControls}${svgPanels}</div></div>`
    }
